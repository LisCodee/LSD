...
def login(request):
    ...
    if request.method == "GET":
        ...
        try:
            ...
            # 存在则进行密码匹配，取出随机字符串
            s = models.UserStr.objects.get(user_name=name).str
            # 创建MD5对象
            m = hashlib.md5()
...
def all_salesman(request):
    ...
    if request.method == "GET":
        ...
        for user in users:
            # 获取姓名和电话
            response.append({'user_name':user.user_name,'telephone':user.telephone})
...
def del_salesman(request):
    ...
    if request.method == "POST":
        ...
        user_name = body['user_name']
        user = models.User.objects.filter(user_name=user_name)
        userstr = models.UserStr.objects.filter(user_name=user_name)
        userstr.delete()
        user.delete()
        response['status'] = True
        return JsonResponse(data=response, safe=False)
...
def search_with_salesman(request):
    ...
    if request.method == "GET":
        ...
        for project_id in projects_id:
            single_project = {}
            project = models.Project.objects.filter(project_id=project_id.project_id, effective=0).first()
            single_project['project_id'] = project.project_id
            single_project['project_name'] = project.project_name
            single_project['time'] = project.time
            users = models.UserProject.objects.filter(project_id=project_id.project_id)
            peoples = []
            # 循环放入负责人姓名
            for user in users:
...
def add_project(request):
    ...
    if request.method == "POST":
        ...
        telephone = body["telephone"]
        models.Project.objects.create(project_name=project_name, source=source, introduction=introduction,
                                                contacts=contacts, telephone=telephone)
        project_id = models.Project.objects.filter(project_name=project_name)[0].project_id
        name = body['user_name']
        models.UserProject.objects.create(project_id=project_id, user_name=name)
        response['status'] = True
        return JsonResponse(data=response, safe=False)
...
def single_project_record(request):
    ...
    if request.method == "GET":
        ...
        messages = models.Message.objects.filter(project_id=project_id)
        for message in messages:
            if identity == "salesman":
                message.read = 1
                message.save()          # 将此项目的相关留言全都置为已读
            response = {}
            response['status'] = 1
...
def record(request):
    ...
    if request.method == "POST":
        ...
        models.Message.objects.create(project_id=project_id, content=content,read=0)
        response['status'] = True
        return JsonResponse(data=response, safe=False)
...
def get_report(request):
    ...
    if request.method == "GET":
        ...
        # 数据库查询
        report = models.Report.objects.filter(id=report_id)[0]
...
def add_report(request):
    ...
    if request.method == 'POST':
        ...
        # 插入数据库
        models.Report.objects.create(project_id=project_id, progress=progress,reporter=reporter,report_name=report_name,
    ...
    if request.method == "GET":
        ...
        for project_id in project_ids:
            ...
            for one_pro_message in one_pro_messages:
                ...
                dic['read'] = (one_pro_message.read == 1)   # 1是已读，已读返回true
...
def search_project(request):
    if request.method == 'GET':
        ...
        for project_id in projects_id_with_people:
            projects_with_people.append(models.Project.objects.get(project_id=project_id.project_id))
        ...
        for res in result:
            people = []
            ups = models.UserProject.objects.filter(project_id=res.project_id)
            for up in ups:
                people.append(up.user_name)
            message.append({'project_id':res.project_id,'project_name':res.project_name,'time':res.time,'peoples':people})
        print(message)
